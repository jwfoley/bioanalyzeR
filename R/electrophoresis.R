...
#'
#' @param xml.file The filename of an XML file exported from the Bioanalyzer or TapeStation software. The XML file may be compressed with `gzip`. The filename (without the file extension) is used as the name of the batch.
...
read.electrophoresis <- function(..., fit = "spline") do.call(rbind, lapply(list(...), function(xml.file) {
	...
	if (first.char == gzip.first.char) first.char <- readChar(gzfile(xml.file), 1) # if gzipped, uncompress and try again
