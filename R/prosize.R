...
read.prosize <- function(
	...
	which.ladder <- unique(result$peaks$sample.index)[by(result$peaks, result$peaks$sample.index, function(sample.peaks) nrow(sample.peaks) == nrow(ladder.peaks) && all(sample.peaks$length == ladder.peaks[,1]))]
