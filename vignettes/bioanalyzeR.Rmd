...
```{r}
qplot.electrophoresis(d1000, peak.fill = "yellow")
qplot.electrophoresis(d1000, peak.fill = NA)
```
...
```{r}
qplot.electrophoresis(d1000, region.alpha = 0.5)
qplot.electrophoresis(d1000, region.alpha = NA)
```
...
qplot.electrophoresis(eukrna)
qplot.electrophoresis(eukrna, geom = "area")
```
...
```{r}
qplot.electrophoresis(eukrna, xlim = c(100, 2000), ylim = c(NA, 0.08))
```
...
```{r}
qplot.electrophoresis(eukrna, xlim = c(100, 2000), scales = "free_y")
```
...
```{r}
qplot.electrophoresis(eukrna, log = "x")
qplot.electrophoresis(eukrna, log = "xy")
```
...
```{r}
qplot.electrophoresis(eukrna, y = "fluorescence")
```
...
```{r}
qplot.electrophoresis(eukrna, x = "relative.distance", y = "fluorescence")
```
...
qplot.electrophoresis(ladder1)
qplot.electrophoresis(ladder1, y = "molarity")
```
...
qplot.electrophoresis(d5000)
qplot.electrophoresis(d5000, normalize = T)
```
...
```{r}
qplot.electrophoresis(dilutions, facets = sample.type ~ dilution, peak.fill = NA, region.alpha = NA, scales = "free_y")
```
...
```{r}
qplot.electrophoresis(dilutions, facets = ~ sample.type, color = dilution, peak.fill = NA, region.alpha = NA, scales = "free_y")
```
...
```{r}
sparkline.electrophoresis(d1000)
```

However, we still have the freedom to use other faceting formulas and aesthetics:

```{r}
sparkline.electrophoresis(dilutions, facets = sample.type ~ ., color = dilution, region.alpha = NA)
```
...
qplot.electrophoresis(haloplex, y = "molarity", region.alpha = NA, xlim = c(50, 750)) + geom_vline(xintercept = c(100, 200, 700), linetype = 2)
```
...
stdcrv.mobility(dna1000.regression)
```
...
```{r}
qc.electrophoresis(dna1000, "length")
qc.electrophoresis(dna1000, "concentration")
qc.electrophoresis(dna1000, "molarity")
```
