...
This package imports data from Agilent automated electrophoresis systems (Bioanalyzer, TapeStation, Fragment Analyzer, ZAG DNA Analyzer, Femto Pulse) and includes functions to graph and analyze the data.
...
1. When all the settings are correct, click "Export". ![](export_screenshot.png)
...
peak.summaries
qplot.electrophoresis(subset(d1000, sample.name == "D1000 Ladder"), include.ladder = T) + geom_vline(xintercept = peak.summaries$Median)
```
...
```{r}
smear.dilutions <- subset(d1000, startsWith(as.character(sample.name), "Smear sample"))
qplot.electrophoresis(smear.dilutions, facets = NULL)
smear.dilutions$regions
summarize.region(smear.dilutions, 1:4)
```

Here we see that when the same sample is diluted several times, the summary statistics of its length distribution change very little.

Alternatively, the `summarize.custom` function allows us to set a custom region and compare it across all samples:

```{r}
fragment.dilutions <- subset(d1000, grepl("fragment", as.character(sample.name)))
qplot.electrophoresis(fragment.dilutions)
integrate.custom(fragment.dilutions, 80, 500)
summarize.custom(fragment.dilutions, 80, 500)
```

Now we can see that the length distribution's statistics do become less reliable as the sample becomes too faint to detect clearly, viz. the 1/70 dilution that is measured as 1.6 nM.
...
compare.estimates("TapeStation D5000", "D5000 Ladder", "tapestation", "D5000-Plate-96.xml.gz")
...
* Protein assays, the Bioanalyzer Small RNA assay, and the entire Oligo Pro II system are unsupported.
