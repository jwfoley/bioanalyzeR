...
* A single unified data to contain various DNA and RNA assays, even from different batches.
* Automatic estimation of molecule length, concentration, and molarity for every data point.
* A simple but powerful interface (based on `ggplot2`) to draw professional-quality electropherograms with any combination of variables, faceting, and aesthetics, including elegant sparklines.
* Functions to automatically compute and compare any variable of interest within all samples, peaks, or regions, as well as shortcuts to calculate the DV200 of fragmented RNA and the insert:dimer ratio of Illumina libraries.
* A powerful command-line script to automate these analyses.
...
Exporting TapeStation data requires version 4.1 or higher of the [TapeStation Analysis software](https://explore.agilent.com/Software-Download-TapeStation-Systems). Open your data file (`.D1000`, `.HSD1000`, `.RNA`., `.gDNA`, etc.). You need to export both the metadata (in XML format) and electropherogram data (CSV), so you will generate two similarly named output files from each input file.
...
![](agilent_electropherogram_screenshot.png)
